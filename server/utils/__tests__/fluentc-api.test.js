...
const {
  FLUENTC_FREE_API,
  FLUENTC_PAID_API,
  FLUENTC_API_MAX_REQUEST_SIZE,
} = require('../constants')

const { URLSearchParams } = require('url')

const { usage, translate, parseLocale } = require('../fluentc-api')
...
describe('fluentc api', () => {
  ...
  describe('usage', () => {
    ...
    beforeEach(() => {
      ...
      mock.onPost(`${FLUENTC_FREE_API}/usage`).reply(usageHandler)
      mock.onPost(`${FLUENTC_PAID_API}/usage`).reply(usageHandler)
    })
      ...
      describe('succeeds', () => {
        it('with valid key', async () => {
          ...
          expect(mock.history.post[0].url).toEqual(
            `${freeApi ? FLUENTC_FREE_API : FLUENTC_PAID_API}/usage`
          )
    ...
    describe.each([true, false])('for free api %p', (freeApi) => {
      beforeEach(() => {
        const translateHandler = (config) => {
          if (
            stringByteLength(config.data || '') > FLUENTC_API_MAX_REQUEST_SIZE
          ) {
        ...
        mock.onPost(`${FLUENTC_FREE_API}/translate`).reply(translateHandler)
        mock.onPost(`${FLUENTC_PAID_API}/translate`).reply(translateHandler)
      })
      ...
      describe('succeeds', () => {
        async function singleText(freeApi) {
          ...
          // then
          expect(mock.history.post[0].url).toEqual(
            `${freeApi ? FLUENTC_FREE_API : FLUENTC_PAID_API}/translate`
          )
        ...
        it('with a field larger than request size limit', async () => {
          ...
          const params = {
            ...
            auth_key: authKey,
            target_lang: 'DE',
            text: [faker.lorem.paragraphs(FLUENTC_API_MAX_REQUEST_SIZE / 200)],
          }
          // when
          const result = await translate(params)
            ...
            text: faker.helpers.uniqueArray(
              () =>
                faker.lorem.paragraphs(
                  FLUENTC_API_MAX_REQUEST_SIZE / 200,
              ...
              () =>
                faker.lorem.paragraphs(
                  faker.datatype.number({
                    min: FLUENTC_API_MAX_REQUEST_SIZE / 2000,
                    max: FLUENTC_API_MAX_REQUEST_SIZE / 200,
                  })
                ),
              textLength
            ),
